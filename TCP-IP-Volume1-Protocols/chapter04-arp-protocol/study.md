...
- **功能**：**32 位 IPv4 → 48 位以太网 MAC** 的**动态映射**。
- **设计意义**：链路层不必理解上层协议 → **IP 的平台无关性**。
- **本章重点**：报文格式、**缓存状态机**、代理/免费 ARP、嵌入式配置与 **ACD**。

### L3 / L2 关系（一句话）

网络层路由最终必须**坍缩**为“下一跳的 MAC 封装”；ARP 是 IPv4 在以太网上完成这一坍缩的**主要动态机制**。
