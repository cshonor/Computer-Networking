...
运输层位于应用层与网络层之间，是协议栈中的关键枢纽。网络层提供**主机到主机（Host-to-Host）**的逻辑通信；运输层通过端系统上的**端口 / 套接字**标识进程，提供**进程到进程（Process-to-Process）**的逻辑通信。

### 核心定义与对标

- **报文段（Segment）**：运输层将应用报文（Message）分段并加首部后形成的 PDU。
- **逻辑通信**：对应用进程屏蔽路由器、链路类型等细节；仿佛存在一条**端到端**的逻辑信道。
- **与网络层的关系**：IP 提供**不可靠、尽力而为**的交付。运输层（尤其 **TCP**）可在其之上用协议机制实现**有序、可靠**的字节流（丢包、乱序、位错误的处理）。

### 因特网运输层概览

1. **TCP**：面向连接、可靠、流量控制、拥塞控制，语义丰富、开销相对大。  
2. **UDP**：无连接、不可靠、首部极简，仅基础差错检测；适合时延敏感或应用自建可靠性。

运输层通过**端口**将报文段交给正确套接字，这一机制即**多路复用与多路分解**。
