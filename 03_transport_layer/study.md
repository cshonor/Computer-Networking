...
- 客户端下一发送序号：**SEQ = x+1**（握手后首包数据仍从 x+1 起）。
- 假设载荷 **100 字节**：
  - 客户端发：`SEQ = x+1`（携带字节 x+1 … x+100）
  - 服务端确认：`ack = (x+1) + 100 = **x+101**`（已收 x+1～x+100，下次请发 **x+101**）
- 服务端自己的发送序号仍从 **y** 起独立递增，与客户端 **x** 序列**互不干扰**。

**场景 B：服务端 → 客户端发数据**

- 服务端发：`SEQ = y`，假设 **200 字节**
- 客户端确认：`ack = y + 200`（已收 y～y+199，下次期望 **y+200**）

---

#### 序号递进规律（五条）

1. 每发送 **1 字节数据**，本端下一 **SEQ +1**。
2. 发送 **SYN / FIN**（无数据）也 **SEQ +1**。
3. 接收方 **ack** = 对方本段 **SEQ** + 本段已正确接收的**字节数**（含 SYN/FIN 占用的 1）。
4. 建立连接后**不再握手**，靠 **SEQ + ack** 排序、确认、重传。
5. **两端序号独立**：客户端走 **x** 序列，服务端走 **y** 序列。
...
| 步 | 报文 | 说明 |
|----|------|------|
| 1 | 客→服：`SEQ=x` | SYN，握手开始 |
| 2 | 服→客：`SEQ=y`，`ack=x+1` | SYN+ACK |
| 3 | 客→服：`SEQ=x+1`，`ack=y+1` | ✅ 握手结束 |
| 4 | 客→服：`SEQ=x+1`，50 字节 | 首包业务数据 |
| 5 | 服→客：`ack=x+51` | 收齐 x+1～x+50 |
| 6 | 服→客：`SEQ=y`，80 字节 | 服务端反向数据 |
| 7 | 客→服：`ack=y+80` | 收齐 y～y+79 |

**一句话**：握手只同步**双方起点**；之后所有包在各自字节流上**顺序累加**，ack 始终表示「下一期望字节号」。
...
> **核心结论**：运输层实现进程间通信，本质靠**端口标识程序、靠端口完成数据分发**。并行（多进程/多线程/协程）是 CPU 调度与应用层模型；**寻址与投递仍只认端口（及 TCP 四元组）**。

<a id="ch3-2-layers"></a>

### 一、各层级通信对象（必背）

| 层级 | 通信对象 | 寻址依据 | 运输层「看不到」什么 |
|------|----------|----------|-------------------|
| **网络层（IP）** | 主机 → 主机 | **IP 地址** | 主机上具体哪个程序 |
| **运输层（TCP/UDP）** | 进程 → 进程 | **端口号**（+ TCP 四元组） | 进程内的线程、协程 |

- **IP**：把数据送到**另一台电脑**。
- **端口**：在已找到的主机上，定位**具体进程**（经内核映射表投递）。
- **Socket 地址** = **IP + 端口**；创建 Socket 并 `bind`/`connect`，即调用运输层、向内核**注册或指定**端口归属。

**通俗总结**：**IP 找电脑，端口找电脑里的程序**；线程、协程是进程内部执行单元，**不参与**运输层寻址。
...
1. 进程启动后通过 `bind` 等**绑定端口**（或由系统分配临时端口）。
2. 操作系统维护 **端口 ↔ 进程/套接字** 映射表。
3. 报文到达主机 → 内核解析**目的端口**（TCP 还需结合四元组）。
4. 查表将数据**投递**到绑定该端口的进程（的套接字接收队列）。
5. 不同进程绑定**不同端口** → 数据不会串流。

**例**：同一台 PC 同时跑浏览器（访问远端 80/443）、聊天软件、本地后台服务；**入站**到本机的数据靠**本机监听端口**区分；**出站**用源端口区分本机多个进程的并发连接。

**规则不变**：多进程并行时，**端口仍是进程身份标识**；运输层**不感知**并行方式，只认端口号（及 TCP 连接四元组）。
...
| 协议 | 分用依据 | 要点 |
|------|----------|------|
| **UDP** | 常用 **(目的 IP, 目的端口)** | 不同源发往同一目的端口，通常进**同一** UDP 套接字 |
| **TCP** | **四元组**：源 IP、源端口、目的 IP、目的端口 | 每条连接独立**连接套接字**；监听套接字与 `accept` 返回的套接字分离 |

**工程实践（Web 服务器）**

- 在 **80/443** 上：`listen` 的**监听套接字**持续 `accept` 新连接。
- 每个 `accept` 得到**连接套接字**（新四元组）→ **单端口承载海量并发 TCP 连接**。

**易错点**：勿以为 TCP「目的端口对就行」——**源 IP/源端口**必须参与区分连接。
...
> 网络层用 **IP** 实现主机到主机交付；运输层用 **端口**（TCP 还用**四元组**）实现进程到进程分用。内核维护端口映射表完成投递。多进程可各占不同端口，或通过**端口复用**多进程共听一口；多线程共享进程端口，运输层只见进程不见线程。Socket = IP + 端口，是应用使用运输层的 API。
