...
- 典型：**DNS、直播、VoIP、游戏、物联网**；完整版见 [§3.3](./study.md#ch3-3)。
...
- **发送方多路复用**：从多个套接字/进程收集报文，加上运输层首部（**源/目的 IP、端口** 等），交给网络层；首部供对端**解复用**。
- **接收方多路解复用**：根据首部 **IP + 端口**（TCP 还需四元组）把报文段交给**正确套接字**及对应应用进程。
...
![UDP 多路解复用：目的 IP + 目的端口](./assets/udp_demux.png)

| 要点 | 说明 |
|------|------|
| **套接字标识** | **(目的 IP, 目的端口)** 二元组 |
| **分用规则** | 查**目的端口** → 交给对应 UDP 套接字 |
| **与 TCP 区别** | 源 IP/源端口**不同**，只要目的 IP+端口相同 → **同一套接字** |

**Socket 示例（伪代码）**

- **服务端**：`socket(SOCK_DGRAM)` + **`bind` 固定端口**（如 53）。
- **客户端**：`socket(SOCK_DGRAM)`，通常**不显式 bind**，OS 分配临时源端口；只要能到达服务端即可。

**内核映射表示意**

| PID | Socket | 目的 IP | 目的端口 |
|-----|--------|---------|----------|
| 12345 | … | 202.38.64.1 | 80 |
| 12346 | … | 202.38.64.1 | 11010 |

---

#### TCP：面向连接解复用（四元组）

![TCP 多路复用：四元组标识连接](./assets/tcp_mux_4tuple.png)

| 四元组字段 | 作用 |
|------------|------|
| **源 IP** | 区分不同客户端主机 |
| **源端口** | 区分同一主机上的不同客户端进程/连接 |
| **目的 IP** | 本机地址 |
| **目的端口** | 本机服务（如 80/443） |

- 接收主机用四元组**解复用** → 把 IP 数据报交给**唯一对应**的 TCP 套接字。
- **单端口多连接**：服务端可在**同一端口**上同时存在多个 TCP 套接字（四元组不同）；Web 服务器常为**每个客户端/每次连接**创建不同套接字（非持久 HTTP 甚至**每请求一新套接字**）。

| 协议 | 分用依据 | 要点 |
|------|----------|------|
| **UDP** | **(目的 IP, 目的端口)** | 不同源 → **同一** UDP 套接字 |
| **TCP** | **四元组** | 每条连接独立**连接套接字**；**监听套接字** ≠ `accept` 返回的套接字 |
...
| 字段 | 宽度 | 作用 |
|------|------|------|
| 源端口 | 16 bit | 可选（客户端常由系统分配） |
| 目的端口 | 16 bit | 分用至进程 |
| 长度 | 16 bit | UDP 首部 + 数据总长度（字节） |
| 校验和 | 16 bit | 覆盖首部 + 数据 + **伪首部** |
...
| **视频直播/短视频** | 偶发丢包 ≈ 花屏；**不能等重传** |
| **语音 VoIP** | 延迟 **>200ms** 体验骤降 |
