...
**定义**：**Shared Memory Switching** — 整机用**一块共享内存**作转发中心，**所有端口**连到该内存，**CPU 集中控制**转发（**非 ASIC 线速**路径）。

#### 原理（考试原话）

- 输入端口 → **复制到共享内存** → **CPU 查路由** → **复制到输出端口缓存**  
- 转发由 **CPU 集中控制**（非线速 ASIC）

#### 标准流程（四步）

| 步 | 动作 |
|----|------|
| 1 | **输入接收**：端口收包，**中断**通知 CPU |
| 2 | **写入共享内存**：整包**拷贝**到全局共享内存 |
| 3 | **CPU 处理**：读首部，查路由/转发表，定**出端口** |
| 4 | **输出转发**：从共享内存**拷贝**到该**输出端口缓存**，端口发出 |

#### 核心特点

- **共享内存是唯一中转站**；所有端口争用**同一内存带宽**  
- **两次拷贝**：入→内存、内存→出 → 有效带宽常 **≈ 内存带宽 / 2**（读写双向占用）  
- **CPU 集中调度**：**无法线速**；端口多、流量大易卡；**时延大**

#### 优缺点

| | 说明 |
|--|------|
| ✅ | **结构简单、成本低、易实现**；早期低端路由器/交换机 |
| ❌ | **内存瓶颈**：最大转发带宽受内存带宽限制（常记 **≤ M/2**） |
| ❌ | **CPU 瓶颈**：每包经 CPU，**非硬件线速** |
| ❌ | **时延大**：两次拷贝 + CPU 查表 |

**一句话**：**端口→共享内存→CPU 查表→输出缓存；CPU 集中、两次拷贝、带宽减半，低速设备用。**

<a id="ch4-2-switch-os-confusion"></a>

> **易混（非本题）**：OS **IPC 共享内存**（进程通信、零拷贝）≠ **OS swap**（换页到磁盘）≠ **网络设备共享内存交换**（只背本节）。
...
| 结构 | 路径 | 瓶颈 | 并行 | 线速 | 场景 |
|------|------|------|------|------|------|
| **共享内存** | 入→**内存**→CPU→出缓存；**两次拷贝** | **内存带宽/2**、**CPU** | 否 | **否** | 早期低端 |
| **共享总线** | 入→**总线**→出；一对占用 | **总线带宽** | 串行一对 | 中 | 企业中端 |
| **Crossbar** | 入→**交叉点**→出；矩阵 | 调度（iSLIP） | **多对并行** | **是** | 核心/骨干 |

| 对比项 | 内存 | 总线 | Crossbar |
|--------|------|------|----------|
| 控制 | **CPU 集中** | 仲裁 | 硬件交叉开关 |
| 拷贝次数 | **2 次** | 少/直挂总线 | 通常无「整块拷内存」 |
| 带宽公式直觉 | **≤ 内存带宽/2** | = 总线带宽 | 随端口扩展 |
...
| 锚点 | 内容 |
|------|------|
| [#ch4-2-switch-memory](#ch4-2-switch-memory) | 共享内存交换四步 |
