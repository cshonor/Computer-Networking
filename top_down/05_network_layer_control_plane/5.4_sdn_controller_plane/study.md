...
| 模式 | 谁算 | 看见什么 | 起点 |
|------|------|----------|------|
| **传统** | **每台路由器各自**算 | 同 Area **LSDB 一致**（OSPF）；**本地 Dijkstra** | **只能以自己**为起点 |
...
→ Dijkstra 手算：[5.1 §八 Dijkstra](../5.1_routing_algorithm/study.md#ch5-1-dijkstra) · OSPF LSDB+SPF：[5.2 三结论](../5.2_ospf_intra_as_routing/study.md#ch5-2-lsdb-spf-core)
...
| | 传统 | SDN |
|---|------|-----|
| 比喻 | 同 Area **共用一张地图**，**各自**站在本路口算最短路 | 指挥中心看**全城地图**，任意出发点算最优路，**指令下发**各路口 |
| 算路 | 分散在各设备 | **集中在控制器** |
| 转发 | 设备自己查路由表 | 设备**只查流表** |
