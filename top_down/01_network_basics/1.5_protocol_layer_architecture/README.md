...
- **五层**：应运网链物；[PDU](./study.md#ch1-5-pdu) · [TCP/UDP](./study.md#ch1-5-tcp-udp) · [IPv4/以太网帧](./study.md#ch1-5-ip-frame)
- **分层优点**：解耦、易维护、复用、标准化
- **对等层**：同层逻辑对话；实际靠下层封装传递
- **封装**：应用数据 → TCP → IP → **以太网帧（MAC 最外）**
- **应用层报文**：[HTTP/SMTP/SSH 对比](./study.md#ch1-5-app-messages) + 三张结构图
...
| `study.md` | 可背诵完整版 + 封装 + HTTP/SMTP/SSH |
| `demo_code/` | 示例代码 |
