...
| **操作类型** | **1=请求，2=应答** |
| 发送端 MAC / IP | |
| 目标端 MAC / IP | 请求时目标 MAC 常为全 0 |

ARP 本身也封装在**以太网帧**里（类型 **`0x0806`**），**不是** IP 包。

![ARP 报文字段结构](../assets/arp_packet_structure.png)

![ARP 请求 Wireshark 十六进制标注](../assets/arp_request_wireshark_hex.png)

**请求包速记**：目的 MAC **广播** · 类型 `08 06` · **OP=1** · 目标 MAC **00:00:00:00:00:00** · 目标 IP = 要问的地址

→ 章级：[#ch6-arp](../study.md#ch6-arp)
