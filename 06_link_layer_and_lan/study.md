...
![ARP 报文字段结构（以太网头 + ARP）](./assets/arp_packet_structure.png)

| 字段 | 典型值 | 说明 |
|------|--------|------|
| 帧目的 MAC | `ff:ff:ff:ff:ff:ff` | **广播**（请求时） |
| 帧源 MAC | 6 字节 | 发送方网卡 |
| 帧类型 | `0x0806` | **ARP**（非 `0x0800` IP） |
| 硬件类型 | `0x0001` | 以太网 |
| 协议类型 | `0x0800` | IPv4 |
| 硬件/协议长度 | 6 / 4 | MAC / IPv4 |
| **操作 OP** | **1=请求，2=应答** | |
| 发送端 MAC+IP | | 谁问的 |
| 目标 MAC | 请求时常 **全 0** | 还不知道对方 MAC |
| 目标 IP | | 要问的 IP |

**Wireshark 十六进制对照（ARP 请求实例）**

![ARP 请求抓包：广播 MAC、0x0806、OP=1、源/目的 IP](./assets/arp_request_wireshark_hex.png)

```text
ff ff ff ff ff ff          ← 以太网目的MAC：广播
38 68 93 53 00 f2          ← 源MAC
08 06                      ← 类型：ARP
00 01 08 00 06 04 00 01    ← 硬件/协议/长度；OP=1 请求
… 源MAC … c0 a8 2b 7a      ← 192.168.43.122
00 00 00 00 00 00          ← 目标MAC未知（全0）
c0 a8 2b 55                ← 目的IP 192.168.43.85
```
