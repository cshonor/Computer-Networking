...
| 光猫 = 路由器？ | 光猫 **ONT** 光电转换；路由器做 NAT/分发（家宽常光猫+路由一体机） |

---

## 个人总结（背诵版）

**边缘=主机+接入网+边缘路由器（分界）；核心是骨干转发。接入网=主机到边缘路由器（最后一公里）。端系统=客户机+服务器；光猫/出口路由是分界不是应用主机。接入技术：DSL/同轴/光纤/Wi‑Fi/5G；架构：C/S 与 P2P。**
