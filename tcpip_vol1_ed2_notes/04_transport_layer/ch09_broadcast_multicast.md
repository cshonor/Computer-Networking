...
> **交叉引用**：[ch02 地址与子网语义](../01_architecture/ch02_internet_addresses.md#ch02-3) · [ch08 ICMP/MLD](../03_network_layer/ch08_icmpv4_icmpv6.md) · [ch10 UDP 与一对多套接字](ch10_udp.md) · 自顶向下 [§3.3 UDP](../../03_transport_layer/study.md#ch3-3)

继 [ch02 单播与编址](../01_architecture/ch02_internet_addresses.md) 之后，本章把「**一个发送者 → 多个接收者**」在**同一条链路/子网**上的两种核心机制讲透：**广播（broadcast）** 与 **本地组播（local multicast）**。前者由**二层洪泛 + 主机全体收包**驱动；后者用 **D 类 / IPv6 组播前缀** 表达**逻辑组**，并用 **IGMP / MLD** 在路由器上维护**按需转发的软状态**。
...
| **单播** | L3  destination 唯一 | 一对一 | 路由表；与 [ch05 IP](../03_network_layer/ch05_ip.md) 一致 |

单播强调 [端到端](../01_architecture/ch01_introduction.md#ch01-e2e)；广播/组播则在**最后一跳子网**上显著改变「谁必须看这份拷贝」。
...
| **定向广播**（directed / subnet） | **某子网的「主机位全 1」地址** | **历史上可跨网段转发**；现今设备**默认禁止**转发到该子网 | 曾用于「远程唤醒整个子网」；**Smurf** 放大的温床 → 见 [§9.5](#ch09-5) 与 [ch08 §8.7](../03_network_layer/ch08_icmpv4_icmpv6.md#ch08-7) |
...
- 纵深阅读：[ch02 IPv6 与多播前缀](../01_architecture/ch02_internet_addresses.md#ch02-3) · [ch08 ND](../03_network_layer/ch08_icmpv4_icmpv6.md#ch08-5)
...
→ 防火墙与边界策略：[ch07](../03_network_layer/ch07_firewall_nat.md#ch07-7)。
...
| **与 UDP** | ICMP/引导类常见 | **`SO_REUSEADDR`/`SO_REUSEPORT`** + **JoinGroup**：见 [ch10](ch10_udp.md) |
...
→ MLD 报文装在 **ICMPv6**（Type 查询/监听报告类）——与 [ch08 §8.2](../03_network_layer/ch08_icmpv4_icmpv6.md#ch08-2) 同属 **ICMPv6 大家族**。
...
| **与 ICMP 攻防关联** | [ch08 §8.7 ICMP 攻防](../03_network_layer/ch08_icmpv4_icmpv6.md#ch08-7) |
...
| **传输层实务** | TCP/UDP 均可（TCP **仅点对点**语义） | 常见 UDP/ICMP；TCP **无意义**（会话需点对点子网可达） | 常见 **UDP**；见 [**ch10 UDP**](ch10_udp.md)：组播 Join、TTL/跳数限制 |
...
1. **编址与子网**：子网边界如何定义「本子网」「广播给谁」—— [ch02 特殊地址与编址](../01_architecture/ch02_internet_addresses.md#ch02-3)。  
2. **ICMP / MLD / 攻击面**：回声、ND、Smurf—— [ch08](../03_network_layer/ch08_icmpv4_icmpv6.md)。  
3. **套接字与 UDP 工程**：`IP_ADD_MEMBERSHIP`、`IP_MULTICAST_TTL`、[ch10](ch10_udp.md)。

### 下一章

- **[ch10 UDP](ch10_udp.md)** — 组播 socket、跳数限制  
- **[ch08 ICMP](../03_network_layer/ch08_icmpv4_icmpv6.md)** — MLD  
- **[ch07 防火墙](../03_network_layer/ch07_firewall_nat.md#ch07-7)** — 过滤与阈值
