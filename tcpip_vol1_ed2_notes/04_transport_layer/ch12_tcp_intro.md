...
> 对照 UDP：[ch10](ch10_udp.md) · 端到端：[ch01](../01_architecture/ch01_introduction.md#ch01-e2e) · 自顶向下：[§3.1 TCP](../../03_transport_layer/study.md#ch3-1-tcp-conn)
...
→ 深入连接与序号：[ch13](ch13_tcp_connection.md) · [§3.1 握手/SEQ/ACK](../../03_transport_layer/study.md#ch3-1-tcp-seq-full)
...
→ 详解：[ch15](ch15_tcp_dataflow_window.md) · [ch16](ch16_tcp_congestion.md) · [QUICKREF §五条易混](../QUICKREF.md)
...
→ 与 UDP 伪首部对比：[ch10 §10.3](ch10_udp.md#ch10-3)
...
**MSS**：TCP 单次可发最大段载荷，通常 **MTU − IP头 − TCP头**（避免 IP 分片）→ [ch03 MTU](../02_link_layer/ch03_link_layer.md#ch03-8)
...
| 章 | 主题 |
|----|------|
| [ch13](ch13_tcp_connection.md) | 三次握手/四次挥手、状态机、TIME_WAIT |
| [ch14](ch14_tcp_timeout_retransmit.md) | RTO、快速重传 |
| [ch15](ch15_tcp_dataflow_window.md) | 滑动窗口、Nagle、延迟 ACK |
| [ch16](ch16_tcp_congestion.md) | 慢启动、拥塞避免、AIMD |
