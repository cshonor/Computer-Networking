...
→ 分用类型字段：[ch01 §分用](../01_architecture/ch01_introduction.md#ch01-layering)（0x0800 = IPv4）
...
| 术语 | 说明 |
|------|------|
| **自动协商** | 两端协商速率（10/100/1000M）与双工 |
| **PAUSE 帧** | 802.3x 流量控制 |

- **双工不匹配**：一端全双工、一端半双工 → 半双工侧虚假冲突 → 吞吐骤降  
- **WoL**：魔术分组唤醒  
- **PAUSE 流程**：接收缓冲满 → 发 **类型 0x8808** 控制帧，操作码 **0x0001** + 暂停时间 → 发送方暂停队列

**PAUSE 帧要点**：目的 MAC **01:80:C2:00:00:01**，类型 **0x8808**，固定 64 B 结构。
...
交换机 = 多端口网桥；**MAC 表**转发 + **防环**。

| 术语 | 说明 |
|------|------|
| **STP (802.1D)** | 逻辑阻塞端口，无环拓扑 |
| **BPDU** | 交换机间配置报文 |

### STP 四步

1. 选**根网桥**（最小 Bridge ID）  
2. 选**根端口**（到根开销最小）  
3. 选**指定端口**  
4. **阻塞**其余冗余端口  
...
收帧 → 记 **源 MAC–端口** → 查**目的 MAC** → 命中转发，未命中**泛洪**

### BPDU 字段（概览）

协议 ID、版本、类型、标志、根 ID、路径开销、发送桥 ID、端口 ID、Hello/MaxAge/ForwardDelay 等。
...
WAN 常用；无介质争用，侧重**认证与协商**。

| 协议 | 作用 |
|------|------|
| **LCP** | 链路参数（MRU、魔术字、认证类型） |
...
|--|-----|------|
| 握手 | 两次 | 三次 |
| 安全 | **明文** | 挑战-响应，**不传密码**（MD5 等） |

### 建立流程

物理 up → **LCP** → **PAP/CHAP** → **NCP(IPCP)** 分配 IP → 传数据

### PPP 帧（文本化）

```text
0x7E | 0xFF 地址 | 0x03 控制 | 协议 2B | 载荷 | FCS | 0x7E
```

协议字段标识 LCP / IP / IPCP 等。

### 易混

**PPP 要 MAC 吗？** 不需要；点到点仅一个对端，地址常固定 **0xFF**。
...
→ 卷1 ch10 UDP + 本书 ch5：[ch10 UDP](../03_transport_layer/ch10_udp.md) · [§3.3 避免 IP 分片](../../03_transport_layer/study.md#ch3-3)
...
- [ch04 ARP](../03_network_layer/ch04_arp.md) — MAC 与 IP 映射  
- [ch05 IP](../03_network_layer/ch05_ip.md) — 网络层  
